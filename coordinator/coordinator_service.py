...
class CoordinatorService(object):
    ...
    def __init__(self, checkpointing_protocol: str, checkpointing_interval: int):
        ...
        logging.warning("coordinator init over g+")
        self.puller_task = ...
        ...
        while root_set_changed:
            ...
            for worker_id in self.recovery_graph_root_set.keys():
                for op_name in self.recovery_graph_root_set[worker_id].keys():
                    ...
                    if (worker_id, op_name, snapshot_timestamp) in marked_nodes: # q1 不成立
    ...
    async def send_restore_message(self):
        ...
        # to_replay
        logging.warning(f"to_replay is: {to_replay}")
    ...
    async def test_snapshot_recovery(self):
        ...
        # recovery_graph
        # logging.warning(f"self.recovery_graph is: {self.recovery_graph}")
        # recovery_graph_root_set
        # logging.warning(f"self.recovery_graph_root_set before is: {self.recovery_graph_root_set}")
        self.find_recovery_line()
        # logging.warning(f"self.recovery_graph_root_set after is: {self.recovery_graph_root_set}")
    ...
    def process_snapshot_information(self, message):
        ...
        # logging.warning('start store_messages guo!')
        self.store_messages(message)
        # logging.warning('store_messages over guo!')
        # Update root set
    ...
    async def get_metrics(self):
        ...
        if self.amount_of_failures > 0:
            logging.warning(f'total_recovery_time:{self.total_recovery_time}')
    ...
    async def coordinator_controller(self, deserialized_data, resp_adr):
        ...
        match message_type:
            case 'SEND_EXECUTION_GRAPH': # 2g
                ...
                logging.warning("Submitted Stateflow Graph to Workers g+")
