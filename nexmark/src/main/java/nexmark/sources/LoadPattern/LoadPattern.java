...
public abstract class LoadPattern {
    ...
    /**
     * Generates a list of integers by repeatedly fetching the first element from a load pattern.
     * 
     * @param times The number of times to fetch and add the load pattern's first element to the result list.
     * @return A list containing the first elements from the load pattern, fetched 'times' number of times.
     * @throws Exception If there's an interruption during Thread.sleep() or if getLoadPattern() fails.
     */
    public List<Integer> patternList(int times) throws Exception {
        List<Integer> patternList = new ArrayList<>();
        
        for (int i = 1; i <= times; i++) {
            Thread.sleep(4);
            List<Integer> loadPattern = this.getLoadPattern().f1;
            patternList.add(loadPattern.get(0));
        }
        return patternList;
    }
