...
public class LoadPatternGenerator {
    ...
    static LoadPattern getCosineLoadPattern(int query, int experimentLength, boolean useDefaultConfigurations,
        ...
        if (useDefaultConfigurations) {
            ...
            int inputRateMaximumDivergence = params.getInt("input-rate-maximum-divergence", 0);
            int inputRateMean = params.getInt("input-rate-mean");
            int maxNoise = params.getInt("max-noise");
            int enableDynamicInterval = params.getInt("enable_dynamic_interval");
            int enableDynamicRatio = params.getInt("enable_dynamic_ratio");
            loadPattern = new CosineLoadPattern(query, experimentLength, cosinePeriod, inputRateMaximumDivergence, inputRateMean, maxNoise, enableDynamicInterval, enableDynamicRatio);
        }
    ...
    /**
     * Predicts the next load pattern rate using a linear regression model.
     * 
     * @param loadPattern The object containing historical load pattern data
     * @return Predicted next load rate value (as integer)
     * @throws Exception If any error occurs during data retrieval or prediction process
     */
    public static int predictLoadPatterRate(LoadPattern loadPattern) throws Exception{
        long startTime = System.currentTimeMillis();
        
        List<Integer> nums = loadPattern.patternList(10);
        // System.out.println(nums);
        SimpleRegression regression = new SimpleRegression();
        for (int i = 0; i < nums.size(); i++) {
            regression.addData(i, nums.get(i));
        }
        int predictResult = (int) regression.predict(nums.size());
        ...
        result.put("predictRate", predictResult);
        result.put("elapsedTime", elapsedTime);

        Gson gson = new GsonBuilder()
        .setPrettyPrinting()
        .create();
        // 当前目录为IFT/checkmate/scripts
        try (FileWriter file = new FileWriter("../dynamic_params/predict_result.json")) {
            gson.toJson(result, file);
        } catch (Exception e) {
            throw new Exception("Failed to write JSON file", e);
        }
